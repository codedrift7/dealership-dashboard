...
End-to-end flow from login through role-gated actions:
...
Hashes are generated with `genhashes.py` and applied to the database with plain `UPDATE` statements (see [Setup](#setup) below) — nothing here depends on Aiven having a built-in query console; any MySQL client works.

### Demo Credentials

Only a handful of seeded accounts have real bcrypt hashes wired up for demo purposes — the rest of the `UserAccount` table is placeholder data and can't log in.

| Username  | Password  | Role   |
|-----------|-----------|--------|
| admin     | admin123  | Admin  |
| mmoore    | owner123  | Owner  |
| asanchez  | sales123  | Sales  |

> ⚠️ These are demo-only credentials for evaluating the app locally. Never reuse plaintext passwords like this for real accounts — see `genhashes.py` to set your own.
...
├── generatehashes.py      # One-off script to bcrypt-hash plaintext passwords
...
The seed data ships with placeholder hashes. Edit `genhashes.py` with the plaintext passwords you want for each account, run it, then apply the output as `UPDATE` statements:
```bash
python genhashes.py > update_passwords.sql
